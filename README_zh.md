...
> **一个专注于教学的 RAG 架构演练系统，旨在展示为什么绝大多数“RAG 教程”在真实生产环境中都会崩溃。**

*本项目**并非生产框架或部署系统**。它是一个针对真实 RAG 架构设计模式的**概念性教学模拟沙盒**。*
...
AI Model Atlas 是一个**概念设计 + 工程教学系统**，深入剖析了现代 RAG 系统在面对复杂业务场景时的底层运转逻辑。

它将向你展示：
* 真实的 RAG 系统在生产中是如何进行架构设计的
* 为什么初级的“向量检索 ➔ 大模型”管道会轻易失效
* 路由、缓存、质量评测和工作流编排是如何协同工作的
* 系统从原型 Demo 演进到真实业务规模时，会发生哪些设计变化
...
这种极致的简化具有很强的误导性。在真实业务系统中，这种简易设计会瞬间崩溃：
* ❌ **一切查询都退化到向量检索**（工具之间缺乏智能路由逻辑）
* ❌ **每一次查询都会去调用大模型**（缺少语义缓存层）
* ❌ **没有任何质量评测与反馈闭环**
* ❌ **缺少任何故障自愈与重试策略**
* ❌ **没有混合检索策略**（BM25 + 密集向量 + RRF 重排）
* ❌ **系统层缺乏组件间的状态控制或工作流编排**

👉 AI Model Atlas 的存在，就是为了**将这些隐藏在教程背后的系统级架构设计可视化、显性化**。

---

## 🧱 核心架构概念剖析

本系统不是一个简单的聊天机器人，而是一个 **RAG 系统架构设计模拟器**：

### 🧠 第一层 — 智能意图路由 (Level 1 — Intelligence Layer)
* 查询智能路由 (工具选择逻辑)

### 🔍 第二层 — 混合检索层 (Level 2 — Retrieval Layer)
* 混合检索 (BM25 + 密集向量 + RRF 融合重排)
* 关系图谱推理层 (GraphRAG 核心概念)

### ⚡ 第三层 — 系统优化层 (Level 3 — Optimization Layer)
* 语义缓存 (命中时零 LLM 消耗的快速响应通道)

### 🛡️ 第四层 — 容灾与可靠层 (Level 4 — Reliability Layer)
* 量化评测概念 (LLM-as-a-judge 裁判引擎)
