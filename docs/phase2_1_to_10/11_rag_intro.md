...
If you ask an AI about your company's internal product roadmap or your private daily schedule, it will either hallucinate or tell you it doesn't know. This is because the model was not trained with access to that private, up-to-date information.
...
* **RAG LLM (Open-Book Exam)**: You sit in a room, and someone hands you a question. Before you answer, a fast librarian runs into a private library, pulls out the most relevant pages, and places them on your desk. You read those pages and write a better-grounded answer, while still checking that the retrieved evidence is relevant.
...
4. **Generation**: The LLM reads the context and writes a grounded answer, but the result still depends on retrieval quality and prompt design.
...
| **Factuality** | Better grounded when retrieval is relevant; still needs verification. | Moderate (still prone to making things up). |
